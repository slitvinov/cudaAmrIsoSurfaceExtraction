...
static vec3i operator>>(const vec3i v, const int s) {
  vec3i u;
  u.x = v.x >> s;
  u.y = v.y >> s;
  u.z = v.z >> s;
...
struct vec3f {
  vec3f() {}
  vec3f(const vec3i o) : x(o.x), y(o.y), z(o.z) {}
  float x, y, z;
};
...
static bool operator==(const vec3f &a, const vec3f &b) {
...
struct CompareMorton {
  bool operator()(const Cell &a, const uint64_t b) const {
    return a.morton < b;
  }
};

struct AMR {
  AMR(Cell *cells, unsigned long long ncell) : cells(cells), ncell(ncell) {}

  bool findActual(struct Cell *result, const vec3i lower, int level) {
    int f;
    const Cell *it =
        std::lower_bound(cells, cells + ncell,
                         morton(lower.x, lower.y, lower.z), CompareMorton());
    if (it == cells + ncell)
      return false;
    *result = *it;

    f = std::max(level, result->level);
...
static void extract(Cell *cells, unsigned long long ncell, int maxlevel,
                    float iso, Vertex *out, int size,
                    unsigned long long *cnt) {
  int x, y, z, index, i, j, k, ii, dx, dy, dz, ix, iy, iz;
  ...
  AMR amr(cells, ncell);
  for (unsigned long long wid = 0; wid < ncell; wid++) {
...
static void createVertexArray(unsigned long long *cnt,
                              const Vertex *vertices, int nvert, Vertex *vert,
                              int size, int3 *index) {
  int i, j, k, l, id, *tri;
  Vertex vertex;
  for (int tid = 0; tid < nvert; tid++) {
    vertex = vertices[tid];
...
positional:
  ...
  cnt = 0;
  extract(cells, ncell, maxlevel, iso, NULL, 0, &cnt);
  ntri = cnt;
  if (Verbose)
  ...
  cnt = 0;
  extract(cells, ncell, maxlevel, iso, tv, 3 * ntri, &cnt);
  free(cells);
